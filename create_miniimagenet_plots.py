...
def main():
    ...
    e = 500  # number of epochs per task
    t = 20  # tasks
    m = 5  # classes per task

    plot_dir = os.path.join("plots", "miniimagenet")
    ...
    experiments = [f"continual-r{r}" for r in ranks] + [f"parallel-r{r}" for r in ranks] + ["parallel-stock"]

    if not all([os.path.exists(os.path.join(plot_dir, f)) for f in [f"{dataset}_test_acc.pkl", f"{dataset}_params.pkl"]]):
    ...
    df = pd.DataFrame({"Method": ["Continual Rank 1", "Continual Rank 5", "Continual Rank 10", "Parallel Rank 1", "Parallel Rank 5", "Parallel Rank 10", "Parallel ResNet18"],
                       "Task Accuracy Mean": np.concatenate([amean[:3], amean[5:8], [amean[10]]]),
                       "Task Accuracy Standard Deviation": np.concatenate([astd[:3], astd[5:8], [astd[10]]]),
                       "Final Task Parameters": np.concatenate([y[:3], y[5:8] * t, [y[10] * t]])})
    ...
    for i in [0, 1, 2]:
        ax.plot(np.arange(t) + 1, np.mean(params[i, :, :], axis=1), f"{colors[i]}-", label=f"Continual Rank {ranks[i]}")
    ...
    ax.plot(np.arange(t) + 1, np.cumsum(np.mean(params[10, :, :], axis=1)), f"{colors[-1]}", label="Parallel ResNet18")
